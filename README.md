...
- **Friends**: `GET /social/friends`, `GET /social/friends/incoming`, **`GET /social/friends/outgoing`**, `POST /social/friends/request`, `POST /social/friends/accept`.
...
- **Friends**: incoming + **outgoing** pending requests, **Accept**, **share invite** from the screen.

### Not done yet (good next steps)

- **EAS** production builds, App Store / Play Store assets.
- **Friends**: optional “add by username” / cancel outgoing request.
...
Apply DB migrations (including word match + **`PlayerExpoPushToken`**) and re-seed for words: `cd backend && npx prisma migrate deploy && npx prisma db seed`.
