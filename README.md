...
- **Words**: `GET /words/session` — random word deck with sentence / word / emoji hints (languages: `en`, `hr`, `de`, `es` in seed); optional **`category`** query filters the deck. **`POST /words/matches`** (optional **`category`** in body), **`POST /words/matches/join`**, **`POST …/start`**, **`GET …/state`** (includes optional **`venueId`**, **`deckCategory`**), **`GET …/deck`**, **`POST …/coop-guess`**, **`POST …/versus-guess`** (server-validated guess; replaces trust-the-client score increments), **`POST …/leave`**, **`POST …/rematch`** — co-op (shared word index) and versus (race) word matches with 6-char **invite codes** on `GameSession`. Stale **PENDING** word rooms are **cancelled** after a timeout (cleanup job).
